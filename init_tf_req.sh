...
table=`cat iac/mc-static/config.tf | sed -e 's/[{}[:space:]]/''/g' -e 's/"//g' | awk -F"=" '$1=="dynamodb_table" {print $2}' | sed -e 's#//[a-zA-Z]*##g'`
...
aws dynamodb create-table --table-name $table --attribute-definitions AttributeName=LockID,AttributeType=S --key-schema AttributeName=LockID,KeyType=HASH --provisioned-throughput ReadCapacityUnits=1,WriteCapacityUnits=1 --region $region --profile $profile